class AssignedAgentQuoteEditNotifyVendorWorker
  include Sidekiq::Worker
  sidekiq_options :retry => false # job will be discarded immediately if failed

  def perform(property_id, agent_id)
    details = PropertyDetails.details(property_id)[:_source]
    assigned_agent_id = details[:agent_id].to_i
    
    ### Send email only if agent id matches with assigned agent id
    if assigned_agent_id == agent_id.to_i
      vendor_first_name = details[:vendor_first_name]
      vendor_property_address = details[:address]
      agent_first_name = details[:assigned_agent_first_name]
      vendor_email = details[:vendor_email]
      template_data = { vendor_first_name: vendor_first_name, vendor_property_address: vendor_property_address, agent_first_name: agent_first_name }
      destination = nil
      ENV['EMAIL_ENV'] == 'dev' ? destination = 'test@prophety.co.uk' : destination = vendor_email
      destination_addrs = []
      destination_addrs.push(destination)
      client = Aws::SES::Client.new(access_key_id: Rails.configuration.aws_access_key, secret_access_key: Rails.configuration.aws_access_secret, region: 'us-east-1')
      resp = client.send_templated_email({ source: 'alerts@prophety.co.uk', destination: { to_addresses: destination_addrs, cc_addresses: [], bcc_addresses: [], }, tags: [], template: 'assigned_agent_quote_edit_notify_vendor', template_data: template_data.to_json})
      Rails.logger.info("AssignedAgentQuoteEditNotifyVendorWorker_#{resp}")
    end
  end

end

