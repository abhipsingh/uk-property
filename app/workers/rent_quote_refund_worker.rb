class RentQuoteRefundWorker
  ...
  def perform(udprn, parent_quote_id)
    ...
    Rent::Quote.where(udprn: udprn.to_i, parent_quote_id: parent_quote_id).where.not(agent_id: nil).where("status = ?", lost_status).each do |quote|
