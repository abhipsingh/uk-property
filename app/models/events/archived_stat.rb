module Events
  class ArchivedStat
    ...
    def transfer_from_unarchived_stats
      ...
      Trackers::Buyer::ENQUIRY_EVENTS.each_with_index do |event, index|
        archived_enquiries[index] += unarchived_enquiries[index]
        unarchived_enquiries[index] = 0
      end
      archived_enquiries[Trackers::Buyer::ENQUIRY_EVENTS.length] += unarchived_enquiries[Trackers::Buyer::ENQUIRY_EVENTS.length]
      unarchived_enquiries[Trackers::Buyer::ENQUIRY_EVENTS.length] = 0

      archived_views = archived_views + unarchived_views
      archived_value = archived_enquiries[0..Trackers::Buyer::ENQUIRY_EVENTS.length].join(ENQUIRY_SEPERATOR) + VIEWS_SEPERATOR + archived_views.to_s
      set_value(archived_value)

      unarchived_views = 0
      unarchived_value = unarchived_enquiries[0..Trackers::Buyer::ENQUIRY_EVENTS.length].join(ENQUIRY_SEPERATOR) + VIEWS_SEPERATOR + unarchived_views.to_s
