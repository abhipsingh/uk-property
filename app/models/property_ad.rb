class PropertyAd < ActiveRecord::Base
  ...
  def self.ads_info_all_address_levels(response, udprn)
    ...
    types.each do |each_type|
      all_locality_levels.each do |each_locality_level|
        ...
        response["#{each_locality_level.to_s}_#{each_type.downcase}_count"] = MAX_ADS_HASH[each_type] - PropertyAd.where(hash_str: hash_str).where(ad_type: TYPE_HASH[each_type]).count
