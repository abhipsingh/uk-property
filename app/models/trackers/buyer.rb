module Trackers
  ...
  class Agent
    ...
    def buyer_enquiries_for_agents_green_properties(property_id)
      cql = "SELECT * FROM Simple.property_events_buyers WHERE property_id = #{property_id} AND status_id = 1"
      generic_execute_property_events_buyers(property_id, cql, status)
    end
    ...
    def generic_execute_property_events_buyers(property_id, cql, status)
      future = session.execute_async(cql) # fully asynchronous api
      result = []
      present_buyer_id = nil
      new_row = nil
      future.on_success do |rows|
        rows.each do |row|
          if rows['buyer_id'] != present_buyer_id
            present_buyer_id = rows['buyer_id']
            result.push(new_row)
            new_row = {}
    ...
    def set_qualifying_stage_value(new_row)
      ...
      (EVENTS[:qualifying_stage]..(max_true - 1)).to_a.map { |e| new_row[REVERSE_EVENTS[e]] = false }
    end
    ...
    def set_count_confidence_level(new_row, row)
      if REVERSE_EVENTS[row['event']] == :confidence_level
        if new_row["confidence_#{row['message']}"].nil?
          new_row["confidence_#{row['message']}"] = 0
        else
    ...
    def property_enquiry_details(property_id, details)
      ...
      future.on_success do |rows|
        rows.each do |row|
          if rows['date'] != present_date || rows['buyer_id'] != present_buyer_id
            present_buyer_id = rows['buyer_id']
            present_date = rows['date']

            new_row[:views] = new_row[:buyer_views].to_s+'/'+new_row[:total_views]
            new_row[:enquiries] = new_row[:buyer_enquiries].to_s+'/'+new_row[:total_enquiries]
