class User
  ...
  def self.postcode_area_panel_details_cache
    ...
    postcode_areas.each do |postcode_area|
      ...
      queries['registered_branches'] = "SELECT count(id) FROM agents_branches WHERE district ~ '^" + postcode_area + "[0-9]+'"
      ...
      queries['vendor_claimed_property'] = "SELECT count(*) from uk_properties join vendors on vendors.property_id = uk_properties.udprn where uk_properties.district ~ '^" + postcode_area + "[0-9]+'"
      queries['manual_properties'] = "SELECT count(*) from agents_branches_assigned_agents_leads join agents_branches_crawled_properties on agents_branches_assigned_agents_leads.property_id = agents_branches_crawled_properties.udprn AND agents_branches_assigned_agents_leads.owned_property = true AND agents_branches_assigned_agents_leads.district ~ '^" +  postcode_area + "[0-9]+'"
      queries['total_listings'] = "SELECT count(*) from uk_properties WHERE district ~ '^" +  postcode_area + "[0-9]+'"
