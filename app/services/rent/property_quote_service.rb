module Rent
  class PropertyQuoteService
    ...
    def accept_quote_from_agent(agent_id)
      ...
      quote = klass.where(udprn: @udprn.to_i, agent_id: nil).order('created_at DESC').last
      agent_quote = klass.where(udprn: @udprn.to_i, agent_id: agent_id).order('created_at DESC').last
      ...
      if quote && quote.status != won_status && agent_quote && agent
        ...
        agent_quote.save!
        klass.where(udprn: @udprn.to_i).where.not(agent_id: nil).where.not(agent_id: agent_id).destroy_all
    ...
    def agent_quotes
      ...
      if vendor_quote
        ...
        quotes_from_agents.each do |each_agent_quote|
          ...
          aggregate_stats[:deadline_start] = Time.parse((vendor_quote.created_at + klass::MAX_VENDOR_QUOTE_WAIT_TIME).to_s).strftime("%Y-%m-%dT%H:%M:%SZ")
